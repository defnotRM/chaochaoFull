-- ============================================================================
-- CHAOCHAO — AUTOMATED TEST SUITE v2 (แก้ปัญหามองไม่เห็นผล RAISE NOTICE)
-- แทนที่จะ RAISE NOTICE ซึ่งบาง Supabase project ไม่ forward กลับมาโชว์ในหน้า
-- SQL Editor สคริปต์นี้บันทึกผลลง temp table แล้ว SELECT ออกมาดูเป็นตารางจริงๆ
-- ท้ายไฟล์แทน รับประกันว่าเห็นผลแน่นอนไม่ว่า Supabase project จะตั้งค่ายังไง
--
...
CREATE TEMP TABLE test_results (test_no INTEGER, test_name TEXT, result TEXT, detail TEXT);
